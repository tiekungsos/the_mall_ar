...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.messageData.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.message-datas.update", [$messageData->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="from">{{ trans('cruds.messageData.fields.from') }}</label>
                <input class="form-control {{ $errors->has('from') ? 'is-invalid' : '' }}" type="text" name="from" id="from" value="{{ old('from', $messageData->from) }}">
                @if($errors->has('from'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('to') ? 'is-invalid' : '' }}" type="text" name="to" id="to" value="{{ old('to', $messageData->to) }}">
                @if($errors->has('to'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message" id="message">{!! old('message', $messageData->message) !!}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $messageData->status || old('status', 0) === 1 ? 'checked' : '' }}>
